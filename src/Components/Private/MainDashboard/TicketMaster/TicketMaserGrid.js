...
export const TicketMasterGrid = () => {
    ...
    const handelDeleteClick = async () => {
        ...
        try {
            ...
            if (deleteResponse) {
                ...
                const pageToFetch = (tickets.length === 1 && pagination.currentPage > 1)
                                    ? pagination.currentPage - 1
                                    : pagination.currentPage;

                const refetchData = await getAllTickets(pageToFetch, itemsPerPage, searchText);
                if (refetchData) {
                    ...
                    setTickets([]);
                    setPagination({ currentPage: 1, totalPages: 0, totalTickets: 0, limit: itemsPerPage, hasNextPage: false, hasPrevPage: false });
                }
            } else {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const data = await getAllTickets(pagination.currentPage, itemsPerPage, searchText);
                if (data) {
        ...
        fetchData();
    }, [pagination.currentPage, deletePopUpShow, updatePopUpShow, AddPopUpShow, addServicePopUpShow]);
                                          ...
                                          <div className="col-8 col-lg-6 text-end">
                                              <div className="form position-relative">
                                                  ...
                                                  <input
                                                      ...
                                                      className="form-control form-input bg-transparent ps-4"
                                            ...
                                            <table className="table table-striped table-class" id="table-id">
                                                <thead>
                                                    <tr className="th_border">
                                                        ...
                                                        <th className="align_left_td width_tdd">Complaint Details</th>
                                                        <th>Product</th>
                                                        <th>Registered by</th>
                                                        <th>Assign</th>
                                                        <th>Action</th>
                                                    ...
                                                    {tickets.length > 0 ? (
                                                        tickets.map((ticket, index) => (
                                                            <tr className="border my-4" key={ticket._id}>
                                                                ...
                                                                <td className="align_left_td">{ticket?.client?.custName || 'N/A'}</td>
                                                                <td className="align_left_td width_tdd">{ticket?.details}</td>
                                                                <td>{ticket?.product}</td>
                                                                <td>{ticket?.registerBy?.name || 'N/A'}</td>
                                                                <td>
                                                                    {user?.permissions?.includes("createService") || user?.user==='company'?(
                                                                ...
                                                                <td>
                                                                {user?.permissions?.includes("updateTicket") || user?.user==='company'?(
                                                                ...
                                                                {user?.permissions?.includes("deleteTicket") || user?.user==='company'?(
                                ...
                                {!loading && (
                                    <>
                                        ...
                                        {pagination.totalTickets === 0 && (
                                            <div className="text-center my-3 text-muted">No tickets found.</div>
                                        )}
                                    </>
                                )}
